...
export function newDelegation(_delegationId: string): Delegation {
  ...
  delegation.lockedUp = EMPTY_STRING
  return delegation
}
...
export function newWithdrawalRequest(_wrId: string): WithdrawalRequest {
  ...
  wr.wrID = ZERO_BI
  wr.hash = ZERO_BYTES
  wr.withdrawalAmount = ZERO_BI
  wr.withdrawHash = ZERO_BYTES
  wr.withdrawTime = ZERO_BI
  return wr
}
...
export function newLockedUp(_id: string): LockedUp {
  ...
  lockedup.endTime = ZERO_BI
  return lockedup
}


export function loadValidator(_id: string, _txHash: string): Validator | null {
